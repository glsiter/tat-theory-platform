...
function generateSimulatedData(n: number = 485): ParticipantData[] {
  ...
  for (let i = 1; i <= n; i++) {
    ...
    const education = ['high_school', 'bachelor', 'master', 'phd'][
      Math.floor(Math.random() * [0.2, 0.5, 0.25, 0.05].reduce((acc, prob, idx) => 
        Math.random() < prob ? idx : acc + 1, 0))
    ] as ParticipantData['education']
    const income_level = ['low', 'medium', 'high'][
      Math.floor(Math.random() * [0.25, 0.5, 0.25].reduce((acc, prob, idx) =>
        Math.random() < prob ? idx : acc + 1, 0))
    ...
    const extroversion_score = (extroversion_items.talkative + extroversion_items.energetic + 
                               extroversion_items.enthusiastic + extroversion_items.assertive + 
                               extroversion_items.outgoing) / 5
    
    const personality_type = extroversion_score >= 3.0 ? 'extrovert' : 'introvert'
    
    // 生成SERVQUAL权重分配 (基于论文发现的效应)
    let tangibles, reliability, responsiveness, assurance, empathy
    ...
    // 响应性和保证性接近显著但未达到
    responsiveness = Math.floor(Math.random() * 8) + 15  // 15-23%
    assurance = Math.floor(Math.random() * 8) + 15       // 15-23%
    ...
    // 住宿信息
    const hotel_location = ['bodrum', 'kusadasi', 'ayvalik'][Math.floor(Math.random() * 3)] as ParticipantData['hotel_location']
    const stay_duration = Math.floor(Math.random() * 10) + 3 // 3-12天
    const travel_purpose = ['leisure', 'business', 'family'][
      Math.floor(Math.random() * [0.7, 0.15, 0.15].reduce((acc, prob, idx) =>
        Math.random() < prob ? idx : acc + 1, 0))
...
export const descriptiveStatistics = {
  ...
  demographics: {
    age: {
      ...
      std: Math.sqrt(simulatedData.reduce((sum, p) => sum + Math.pow(p.age - 42.5, 2), 0) / simulatedData.length),
    ...
    gender: {
      ...
      malePercentage: (simulatedData.filter(p => p.gender === 'male').length / simulatedData.length * 100).toFixed(1),
      femalePercentage: (simulatedData.filter(p => p.gender === 'female').length / simulatedData.length * 100).toFixed(1)
    },
    personality: {
      ...
      introvertPercentage: (simulatedData.filter(p => p.personality_type === 'introvert').length / simulatedData.length * 100).toFixed(1),
      extrovertPercentage: (simulatedData.filter(p => p.personality_type === 'extrovert').length / simulatedData.length * 100).toFixed(1)
    }
  },
  extroversion: {
    mean: simulatedData.reduce((sum, p) => sum + p.extroversion_score, 0) / simulatedData.length,
    std: Math.sqrt(simulatedData.reduce((sum, p) => sum + Math.pow(p.extroversion_score - 3.0, 2), 0) / simulatedData.length),
  ...
  servqualWeights: {
    overall: {
      tangibles: simulatedData.reduce((sum, p) => sum + p.service_quality_weights.tangibles, 0) / simulatedData.length,
      reliability: simulatedData.reduce((sum, p) => sum + p.service_quality_weights.reliability, 0) / simulatedData.length,
      responsiveness: simulatedData.reduce((sum, p) => sum + p.service_quality_weights.responsiveness, 0) / simulatedData.length,
      assurance: simulatedData.reduce((sum, p) => sum + p.service_quality_weights.assurance, 0) / simulatedData.length,
      empathy: simulatedData.reduce((sum, p) => sum + p.service_quality_weights.empathy, 0) / simulatedData.length
    },
    introverts: (() => {
      const introverts = simulatedData.filter(p => p.personality_type === 'introvert')
      return {
        tangibles: introverts.reduce((sum, p) => sum + p.service_quality_weights.tangibles, 0) / introverts.length,
        reliability: introverts.reduce((sum, p) => sum + p.service_quality_weights.reliability, 0) / introverts.length,
        responsiveness: introverts.reduce((sum, p) => sum + p.service_quality_weights.responsiveness, 0) / introverts.length,
        assurance: introverts.reduce((sum, p) => sum + p.service_quality_weights.assurance, 0) / introverts.length,
        empathy: introverts.reduce((sum, p) => sum + p.service_quality_weights.empathy, 0) / introverts.length
      }
    })(),
    extroverts: (() => {
      const extroverts = simulatedData.filter(p => p.personality_type === 'extrovert')
      return {
        tangibles: extroverts.reduce((sum, p) => sum + p.service_quality_weights.tangibles, 0) / extroverts.length,
        reliability: extroverts.reduce((sum, p) => sum + p.service_quality_weights.reliability, 0) / extroverts.length,
        responsiveness: extroverts.reduce((sum, p) => sum + p.service_quality_weights.responsiveness, 0) / extroverts.length,
        assurance: extroverts.reduce((sum, p) => sum + p.service_quality_weights.assurance, 0) / extroverts.length,
        empathy: extroverts.reduce((sum, p) => sum + p.service_quality_weights.empathy, 0) / extroverts.length
      }
    })()
  }
}
